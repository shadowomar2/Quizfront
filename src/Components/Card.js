...
function Card(props) {
  ...
  return (
    <MDBCard style={{height: '35rem', maxWidth: '42rem', backgroundColor: '#222222' }} className="shadow p-3 mb-5 bg-white rounded">
      <MDBCardImage style={{minHeight: '15rem'}} src={props.imgSrc} position='top' alt='...'   onMouseEnter={handleMouseEnter}
  onMouseLeave={handleMouseLeave} />
      <MDBCardBody className="bg-light">
        <MDBCardTitle className="fw-bold mb-3">{props.name}</MDBCardTitle>
        <MDBCardText   >{"Email :"+props.email.slice(0, 40)}{props.email.length > 40 ? '...' : ''}</MDBCardText>
        <div className="d-flex justify-content-between align-items-center">
          <p className="m-0 fw-bold">{"ID :"+props.id}</p>
